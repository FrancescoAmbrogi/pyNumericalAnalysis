"""
Gauss elimination part 2

We need to check if the Pivot element is zero,
because of course we do not want to divide by zero.
...
for k in range(n-1): # until the row before the last
    ...
    if A[k,k] == 0:
        ...
        # What we do now is we swap two rows
        for j in range(n):
